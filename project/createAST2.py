...
def parseStart(token):
    global ast
    global blockParent
    ast.add_node("Start")  # root node
    blockParent = 'Start'
    # Parse for Block
    if (parseBlock(token)):
        if (match(token[p].kind, 'endProgram')):
            ast.add_node(str(token[p].lineNum) + ',' + token[p].character, "Block1")
...
def parseBlock(token):
    ...
    if token[p].character in parseBlockFirstSet:
        ...
        if (match(token[p].character, '{')):
            ast.add_node(str(token[p].lineNum) + ',' + token[p].character, "Block" + str(blockNum))
            brackNum = brackNum + 1
            p = p + 1
            if (parseStatementList(token)):
                if (match(token[p].character, '}')):
                    ast.add_node(str(token[p].lineNum) + ',' + token[p].character, "Block" + str(maxBlock))
                    maxBlock = maxBlock - 1
                    p = p + 1
                    return True
...
def parseStatementList(token):
    ...
    if token[p].kind in statementListFirstSet or token[p].character in statementListFirstSet:
        ...
        ast.add_node("StatementList" + str(stmtListNum), "Block" + str(blockNum))
...
def parseStatement(token):
    ...
    if token[p].kind in statementListFirstSet or token[p].character in statementListFirstSet:
        # ast.create_node("Statement", "statement" + str(stmtNum), parent='block' + str(blockNum))
        stmtNum = stmtNum + 1
        ast.add_node("Statement" + str(stmtNum), "StatementList" + str(stmtListNum))
...
def parsePrintStatement(token):
    ...
    if token[p].character in printStatementFirstSet:
        ...
        ast.add_node("PrintStmt" + str(printStmtNum), "Statement" + str(stmtNum))
        if (match(token[p].character, 'print')):
            ast.add_node(str(token[p].lineNum) + ',' + token[p].character, "PrintStmt" + str(printStmtNum))
            p = p + 1
            if (match(token[p].character, '(')):
                ast.add_node(str(token[p].lineNum) + ',' + token[p].character, "PrintStmt" + str(printStmtNum))
                p = p + 1
...
def parseAssignmentStatement(token):
    ...
    if token[p].kind in printStatementFirstSet:
        ...
        ast.add_node("AssignmentStmt" + str(assignNum), "Statement" + str(stmtNum))
        idParent = "AssignmentStmt" + str(assignNum)
        if (parseId(token)):
            if (match(token[p].character, '=')):
                ast.add_node(str(token[p].lineNum) + ',' + token[p].character, "AssignmentStmt" + str(assignNum))
                p = p + 1
                exprParent = "AssignmentStmt" + str(assignNum)
                if (parseExpr(token)):
                    return True
            elif (token[p].character is not '=' or token[p].character is not '+'):
                print("Error on line " + str(token[
                                                 p].lineNum) + ". Expecting '=' or '+' or there needs to be a variable type ['string', 'int', 'boolean'] before, got " +
                      token[p].character + ".")
...
def parseVarDecl(token):
    ...
    if token[p].kind in varDeclFirstSet:
        ...
        ast.add_node("VarDecl" + str(varDeclNum), "Statement" + str(stmtNum))
        if (match(token[p].kind, 'type')):
            ast.add_node(str(token[p].lineNum) + ',' + token[p].character, "VarDecl" + str(varDeclNum))
            p = p + 1
            if (parseId(token)):
                return True
...
def parseWhileStatement(token):
    ...
    if token[p].character in whileStatementFirstSet:
        whileNum = whileNum + 1
        ast.add_node("WhileStmt" + str(whileNum), "Statement" + str(stmtNum))
        if (match(token[p].character, 'while')):
            ast.add_node(str(token[p].lineNum) + ',' + token[p].character, "WhileStmt" + str(whileNum))
            p = p + 1
...
def parseIfStatement(token):
    ...
    if token[p].character in ifStatementFirstSet:
        ...
        ast.add_node("ifStmt" + str(ifStmtNum), "Statement" + str(stmtNum))
        if (match(token[p].character, 'if')):
            ast.add_node(str(token[p].lineNum) + ',' + token[p].character, "ifStmt" + str(ifStmtNum))
            p = p + 1
...
def parseExpr(token):
    ...
    if (token[p].character in exprFirstSet or token[p].kind in exprFirstSet):
        ...
        ast.add_node("Expr" + str(exprNum), exprParent)
...
def parseIntExpr(token):
    ...
    if (token[p].kind in intExprFirstSet):
        ...
        ast.add_node("IntExpr" + str(intExprNum), "Expr" + str(exprNum))
        if (match(token[p].kind, 'digit')):
            ast.add_node(str(token[p].lineNum) + ',' + token[p].character, "IntExpr" + str(intExprNum))
            digitBool = True
            p = p + 1
...
def parseStringExpr(token):
    ...
    if (token[p].character in strExprFirstSet):
        ...
        # ast.add_node("stringExpr" + str(strExprNum), "Expr" + str(exprNum))
        # Changed to ast.add_node("stringExpr" + str(stringExprNum), "Expr" + str(exprNum))
        ast.add_node("stringExpr" + str(stringExprNum), "Expr" + str(exprNum))
        if (match(token[p].character, '"')):
            ast.add_node(str(token[p].lineNum) + ',' + token[p].character, "stringExpr" + str(stringExprNum))
            p = p + 1
...
def parseBooleanExpr(token):
    ...
    if (token[p].character in boolExprFirstSet):
        ...
        ast.add_node("BoolExpr" + str(boolExprNum), boolExprParent)
        originalBoolExprNum = boolExprNum
        if (match(token[p].character, '(')):
            ast.add_node(str(token[p].lineNum) + ',' + token[p].character, "BoolExpr" + str(boolExprNum))
            p = p + 1
            exprParent = "BoolExpr" + str(boolExprNum)
            if (parseExpr(token)):
                if (match(token[p].kind, 'compare')):
                    ast.add_node("BoolOp" + str(boolOpNum), "BoolExpr" + str(boolExprNum))
                    ast.add_node(str(token[p].lineNum) + ',' + token[p].character, "BoolOp" + str(boolOpNum))
                    boolOpNum = boolOpNum + 1
                    p = p + 1
                    exprParent = "BoolExpr" + str(boolExprNum)
                    if (parseExpr(token)):
                        if (match(token[p].character, ')')):
                            ast.add_node(str(token[p].lineNum) + ',' + token[p].character,
                                         "BoolExpr" + str(originalBoolExprNum))
                            p = p + 1
                            return True

                    
        elif (match(token[p].kind, 'boolval')):
            ast.add_node("BoolVal" + str(boolValNum), "BoolExpr" + str(boolExprNum))
            ast.add_node(str(token[p].lineNum) + ',' + token[p].character, "BoolVal" + str(boolValNum))
            p = p + 1
            return True
...
def parseId(token):
    ...
    if (token[p].kind in idFirstSet):
        ...
        ast.add_node("Id" + str(idNum), idParent)
        if (parseChar(token)):
            return True
...
def parseCharList(token):
    ...
    if (token[p].kind in charListFirstSet):
        ...
        ast.add_node("CharList" + str(charListNum), "stringExpr" + str(stringExprNum))
        charParent = "CharList" + str(charListNum)
        if (match(token[p].kind, 'char')):
            ast.add_node(str(token[p].lineNum) + ',' + token[p].character, "CharList" + str(charListNum))
            p = p + 1
            if (parseCharList(token)):
                return True
    else:
...
def parseChar(token):
    ...
    if (token[p].kind in charFirstSet):
        ast.add_node('char' + str(charNum), charParent)
        if (match(token[p].kind, 'char')):
            ast.add_node(str(token[p].lineNum) + ',' + token[p].character, 'char' + str(charNum))
            p = p + 1
            return True
